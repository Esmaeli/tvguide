...
def main():
    ...
    with open('index.html', 'w', encoding='utf-8') as output_file:
        ...
        output_file.write('document.addEventListener("DOMContentLoaded", function() {\n')
        output_file.write('    const navLinks = document.querySelectorAll(".nav-links a");\n')
        output_file.write('    const eventCards = document.querySelectorAll(".event-cards .card");\n')
        output_file.write('\n')
        output_file.write('    navLinks.forEach(link => {\n')
        output_file.write('        link.addEventListener("click", function(event) {\n')
        output_file.write('            event.preventDefault();\n')
